namespace Service
{
    ...
    public class StudentService
    {
        ...
        public List<Enrollment> GetEnrollmentSchedule(string studentId, ref List<string> errors)
        {
            if (string.IsNullOrEmpty(studentId))
            {
                errors.Add("Invalid student id");
            ...
            /*
            List<Enrollment> studEnrollment = this.repository.GetEnrollments(studentId);
            List<Enrollment> enrollmentSched = this.repository.GetEnrollmentSchedule(studentId);
            /*
            for (int i = 0; i < enrollmentSched.Count; i++)
            {
                if (enrollmentSched[i].CourseTitle.Equals("ha"))
                {

                }
            }*/

                return this.repository.GetEnrollmentSchedule(studentId);
        }
