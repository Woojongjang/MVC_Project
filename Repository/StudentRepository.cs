namespace Repository
{
    ...
    public class StudentRepository : BaseRepository, IStudentRepository
    {
        ...
        private const string GetStudentScheduleProcedure = "GetStudentSchedule";
        private const string GetStudentEnrollmentListProcedure = "GetStudentScheduleList";
        ...
        public List<Enrollment> GetEnrollmentSchedule(string studentId)
        {
            var conn = new SqlConnection(ConnectionString);
            var scheduleList = new List<Enrollment>();

            try
            {
                var adapter = new SqlDataAdapter(GetStudentEnrollmentListProcedure, conn)
                {
                    SelectCommand =
                    {
                        CommandType = CommandType.StoredProcedure
                ...
                for (var i = 0; i < dataSet.Tables[0].Rows.Count; i++)
                {
                    var studentSched = new Enrollment
                    {
                        StudentId = studentId,
                        ...
                        InstructName = dataSet.Tables[0].Rows[i]["first_name"].ToString() + dataSet.Tables[0].Rows[i]["last_name"].ToString(),
                        ...
                        Day = dataSet.Tables[0].Rows[i]["schedule_day"].ToString(),
                        Time = dataSet.Tables[0].Rows[i]["schedule_time"].ToString(),
                        Grade = "N/A",
                        GradeValue = (float)-1
                    };
                    scheduleList.Add(studentSched);
                }
            }
            catch (Exception e)
            {
        ...
        public List<Enrollment> GetEnrollments(string studentId)
        {
            ...
            try
            {
                var adapter = new SqlDataAdapter(GetStudentScheduleProcedure, conn)
                {
                    SelectCommand =
                    {
                        CommandType = CommandType.StoredProcedure
                ...
                for (var i = 0; i < dataSet.Tables[0].Rows.Count; i++)
                {
                    String gradestring = dataSet.Tables[0].Rows[i]["grade"].ToString();
                    var studentSched = new Enrollment
                    {
                        StudentId = studentId,
                        ScheduleId = (int)dataSet.Tables[0].Rows[i]["schedule_id"],
                        ...
                        Day = dataSet.Tables[0].Rows[i]["schedule_day"].ToString(),
                        Time = dataSet.Tables[0].Rows[i]["schedule_time"].ToString(),
                        Grade = String.IsNullOrEmpty(gradestring) ? "N/A" : gradestring,
                        GradeValue = (float) -1
                    };
                    scheduleList.Add(studentSched);
                }
